...
void colorBar_8bit(int width, int hight)
{
	...
	for(i = 0; i < width; i++)
	{
		for(j = 0;j < hight; j++)
		{
			if(i < (width/8))
			{
				pYuvBuff[i + j*width] = 255 / 8; continue;
				
			}
			if(i < ((width/8) * 2))
			{
				pYuvBuff[i + j*width] = 255 / 8 * 2;continue;
				
			}
			if(i < ((width/8) * 3))
			{
				pYuvBuff[i + j*width] = 255 / 8 * 3;continue;
			}
			if(i < ((width/8) * 4))
			{
				pYuvBuff[i + j*width] = 255 / 8 * 4;continue;
			}
			if(i < ((width/8) * 5))
			{
				pYuvBuff[i + j*width] = 255 / 8 * 5;continue;
			}
			if(i < ((width/8) * 6))
			{
				pYuvBuff[i + j*width] = 255 / 8 * 6;continue;
			}
			if(i < ((width/8) * 7))
			{
				pYuvBuff[i + j*width] = 255 / 8 * 7;continue;
			}
			if(i < width)
			{
				pYuvBuff[i + j*width] = 255;continue;
			}
		}
	}
...
void colorBar_16bit(int width, int hight)
{
	...
	for(i = 0; i < pixWidth; i++)
	{
		for(j = 0;j < hight; j++)
		{
			if(i < (pixWidth/8))
			{
				pPixYuvBuff[i + j*pixWidth] = 255 / 8; continue;
				
			}
			if(i < ((pixWidth/8) * 2))
			{
				pPixYuvBuff[i + j*pixWidth] = 255 / 8 * 2;continue;
				
			}
			if(i < ((pixWidth/8) * 3))
			{
				pPixYuvBuff[i + j*pixWidth] = 255 / 8 * 3;continue;
			}
			if(i < ((pixWidth/8) * 4))
			{
				pPixYuvBuff[i + j*pixWidth] = 255 / 8 * 4;continue;
			}
			if(i < ((pixWidth/8) * 5))
			{
				pPixYuvBuff[i + j*pixWidth] = 255 / 8 * 5;continue;
			}
			if(i < ((pixWidth/8) * 6))
			{
				pPixYuvBuff[i + j*pixWidth] = 255 / 8 * 6;continue;
			}
			if(i < ((pixWidth/8) * 7))
			{
				pPixYuvBuff[i + j*pixWidth] = 255 / 8 * 7;continue;
			}
			if(i < pixWidth)
			{
				pPixYuvBuff[i + j*pixWidth] = 255;continue;
			}
		}
	}
